...
}:
{
  home.file.".config/doom/config.el".text = ''
    ...
    (custom-set-faces!
      ...
      '(font-lock-comment-face :family "${codeFontName}" :foreground "${palette.rosewater.hex}" :size ${toString(codeFontSize)}.0 :slant italic)
      '(line-number :foreground "${palette.subtext0.hex}")
      '(line-number-current-line :foreground "${palette.peach.hex}")
      )
  '';
}
